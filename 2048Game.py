...
class Game(Frame):
    def __init__(self):
        ...
        self.command = {c.KEY_UP:logic.move_up, c.KEY_DOWN:logic.move_down, c.KEY_LEFT:logic.move_left,
    ...
    def key_down(self,event):
        ...
        if key in self.command:
            self.matrix, changed = self.command[key](self.matrix)
            if changed:
                logic.add_new_2(self.matrix)
                self.update_grid_cells()
                changed = False
                ...
                if logic.get_current_state(self.matrix) == 'LOST':
                    self.grid_cells[1][1].configure(text = "You",bg = c.BACKGROUND_COLOR_CELL_EMPTY)
                    self.grid_cells[1][2].configure(text = "Lose!",bg = c.BACKGROUND_COLOR_CELL_EMPTY)
                if logic.get_current_state(self.matrix) == 'GAME NOT OVER':
                    logic.add_new_2(self.matrix)
                    self.update_grid_cells()
